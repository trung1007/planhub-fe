...
import { ActionProjectInput, ActionProjectSchema } from "@/schemas/project.schema";
...
const ProjectModal: React.FC<ProjectModalProps> = ({
    ...
    const handleEdit = (data: ActionProjectInput) => {
        if (currentUser && selectedProject) {
            ...
            mutationEdit({ id: selectedProject.id, data: payload }, {
                onSuccess: () => {
            ...
            <form onSubmit={handleSubmit(handleSave)} className="p-5 flex flex-col gap-4">
                ...
                <FormRow label="Project name" error={errors.name?.message}>
                    <Controller
                        name="name"
                        control={control}
                        render={({ field }) => <Input {...field} placeholder="Project name" />}
                    />
                </FormRow>
                ...
                <FormRow label="Project code" error={errors.code?.message}>
                    <Controller
                        name="code"
                        control={control}
                        render={({ field }) => <Input {...field} placeholder="Project code" />}
                    />
                </FormRow>
