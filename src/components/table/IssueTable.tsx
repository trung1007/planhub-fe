...
const IssueTable = () => {
  ...
  const handleSelectOne = (id: number, name: string, checked: boolean) => {
    if (checked) setSelectedIssues((prev) => [...prev, { id, name }]);
    else setSelectedIssues((prev) => prev.filter((x) => x.id !== id));
  };
  ...
  const columns: ColumnsType<any> = [
    {
      title: (
        <Checkbox
          checked={allIds > 0 && selectedIssues.length === allIds}
          indeterminate={
            selectedIssues.length > 0 && selectedIssues.length < allIds
          }
